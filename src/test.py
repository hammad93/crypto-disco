...
from PySide6.QtWidgets import QApplication, QWizard, QPushButton, QLineEdit, QPlainTextEdit
...
class TestCryptoDisco(unittest.TestCase):
    ...
    def test_repair(self, test_file='pismis-24-mini-poster.png'):
        ...
        ecc_result = ecc.generate_ecc(input_path=src_path, output_path=self.tests_dir)
        self.assertTrue(ecc_result, "ECC result should be True")
        # see if it generated the ecc file
        ecc_path = os.path.join(self.tests_dir, f'{src_path}.txt')
        ...
        with patch('PySide6.QtWidgets.QFileDialog.getOpenFileName') as mock_open, \
            ...
            mock_dir.return_value = self.tests_dir
            # 1. Open Wizard
            self.window.show()
            ...
            expected_output = os.path.join(self.tests_dir, test_file)
            QTest.qWait(5 * 1000)
            QTest.qWait(5 * 1000)
            QTest.qWait(5 * 1000)
            QTest.qWait(5 * 1000)
            ...
            QTest.qWait(5 * 1000)
            QTest.qWait(5 * 1000)
            progress_text_edit = current_page.findChild(QPlainTextEdit)
            if progress_text_edit:
                text_content = progress_text_edit.toPlainText()
                self.assertIn("Repair complete", text_content)
